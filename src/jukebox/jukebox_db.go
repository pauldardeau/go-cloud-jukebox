...
func (jukeboxDB *JukeboxDB) enter() bool {
    ...
    //TODO: make connection to sqlite datatbase file
    //jukeboxDB.db_connection = sqlite3.connect(jukeboxDB.metadata_db_file_path)
    if jukeboxDB.open() {
        if jukeboxDB.db_connection != nil {
...
func (jukeboxDB *JukeboxDB) create_table(sql string) bool {
    if jukeboxDB.db_connection != nil {
...
func (jukeboxDB *JukeboxDB) have_tables() bool {
   ...
   if jukeboxDB.db_connection != nil {
      ...
      cursor = jukeboxDB.db_connection.cursor()
      cursor.execute(sql)
...
func (jukeboxDB *JukeboxDB) retrieve_songs(artist string,
    ...
    if jukeboxDB.db_connection != nil {
        sql := `
            ...
            album_uid FROM song
        `
	stmt, err_stmt := jukeboxDB.db_connection.Prepare(sql)
	if err_stmt != nil {
            return nil
        }
        sql += jukeboxDB.sql_where_clause()
        //if len(artist) > 0:
...
func (jukeboxDB *JukeboxDB) delete_song(song_uid string) bool {
   ...
   if jukeboxDB.db_connection != nil {
	 ...
	 tx.Commit()
         was_deleted = true
         //fmt.Println("error deleting song: " + e.args[0])
      }
   }
